...
func (env *Env) ExecDDL(opts *ExecOpts, p *prog.Prog) (output []byte, info *ProgInfo, hanged bool, err0 error) {
	// Copy-in serialized program.
	progSize, err := p.SerializeForExec(env.in)
	if err != nil {
		err0 = fmt.Errorf("failed to serialize: %v", err)
	...
	// add new scheduling method: deadline
	...
	// tid := syscall.Gettid()
	// _, _, err1 := syscall.Syscall6 (
	// 	unix.SYS_SCHED_SETSCHEDULER,
	// 	0,
	// 	SCHED_BATCH,
	// 	uintptr(unsafe.Pointer(
	// 	&SchedParam{
	// 		SchedPrio: int32(rand.Intn(100)),
	// 	})),
	// 	0,
	// 	0,
	// 	0,
	// )
	// if err1 != 0 {
	// 	fmt.Errorf("Error setting scheduler")
	// 	return
	// }

	_, _, err2 := syscall.Syscall6(
		...
		uintptr(
			unsafe.Pointer(
				&SchedAttr{
					...
					Policy:   SCHED_BATCH,
					Deadline: p.DDLAttr.Deadline,
					Period:   p.DDLAttr.Period,
					Runtime:  p.DDLAttr.Runtime,
				})),
		0,
		0,
		0,
	...
	if err2 != 0 {
		fmt.Errorf("Error setting scheduling attribute")
		return
	}
...
func (env *Env) parseOutput(p *prog.Prog) (*ProgInfo, error) {
	...
	for i := uint32(0); i < ncmd; i++ {
		...
		if err != nil {
			fmt.Errorf("err is not nil")
			return nil, err
		}
		inf.Comps = comps

		inf.Stat.LockStat = reply.lockStat
	}
	if len(extraParts) == 0 {
		return info, nil
	}

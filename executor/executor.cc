...
static bool flag_lock = true;
static bool flag_collect_stat = true;
...
void write_stat(stat_t* stat, uint32* stat_pos) {
	*stat_pos = stat->lock_stat;
	// *(stat_pos + 1) = stat->interrupt_stat;
}
...
void write_call_output(thread_t* th, bool finished)
{
	...
	uint32* stat_pos = write_output(0);
	if (flag_comparisons) {
	...
	if (flag_collect_stat) {
		write_stat(&th->stat, stat_pos);
	}

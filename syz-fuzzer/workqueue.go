...
func (wq *WorkQueue) enqueue(item interface{}) {
	...
	case *WorkTaskTriage:
		wq.taskTriage = append(wq.taskTriage, item)
	default:
...
func (wq *WorkQueue) dequeue() (item interface{}) {
	...
	} else if len(wq.taskTriage) != 0 {
		last := len(wq.taskTriage) - 1
		item = wq.taskTriage[last]
		wq.taskTriage = wq.taskTriage[:last]
	}
	wq.mu.Unlock()
	if wantCandidates {
		select {

...
func RunManager(cfg *mgrconfig.Config, target *prog.Target, sysTarget *targets.Target, syscalls []int) {
	...
	log.Logf(0, "create http server and collect used files...")
	mgr.initHTTP()
	mgr.collectUsedFiles()

	// Create RPC server for fuzzers.
	log.Logf(0, "start rpc server...")
...
func (mgr *Manager) runInstance(index int) (*Crash, error) {
	log.Logf(0, "XXXXXXXXXXXXXXXXXX runInstance XXXXXXXXXXXXXXX")
	mgr.checkUsedFiles()
...
func (mgr *Manager) collectUsedFiles() {
	...
	log.Logf(0, "FuzzerBin: %v\n SyzExecProgBin: %v\n, SyzExecutorBin: %v\n, SSHKey: %v", cfg.SyzFuzzerBin, cfg.SyzExecprogBin, cfg.SyzExecutorBin, cfg.SSHKey)

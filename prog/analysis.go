...
// analyze analyzes the program p up to but not including call c.
func analyzeRel(ct *ChoiceTable, rg *ProgTempRelationGraph, corpus []*Prog, p *Prog, c *Call) *RelState {
	s := newRelState(p.Target, ct, rg, corpus)
	resources := true
	for _, c1 := range p.Calls {
		if c1 == c {
			resources = false
...
func newRelState(target *Target, ct *ChoiceTable, rg *ProgTempRelationGraph, corpus []*Prog) *RelState {
	s := &RelState{
		target:    target,
		ct:        ct,
		rg:        rg,
...
func (s *RelState) analyze(c *Call) {
	s.analyzeImpl(c, true)
}

func (s *RelState) analyzeImpl(c *Call, resources bool) {
	ForeachArg(c, func(arg Arg, _ *ArgCtx) {
		switch a := arg.(type) {
		case *PointerArg:
			switch {
